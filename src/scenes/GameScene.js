...
export default class GameScene extends Phaser.Scene {
   ...
   updateAI(delta) {
       ...
       const isPlayerNearEdge = p1DistanceToEdge < aiConfig.strategy.edgeDistance;
       ...
       if (isPlayerNearEdge) {
           ...
           moveX = nx;
           moveY = ny;
       } else if (distToPlayer < aiConfig.strategy.closeDistance) {
       ...
       if (this.aiActionTime <= 0) {
           ...
           this.aiActionTime = aiConfig.updateInterval.min + 
                              Math.random() * (aiConfig.updateInterval.max - aiConfig.updateInterval.min);
           
           // Choose an action based on situation
           if (distToPlayer < aiConfig.strategy.closeDistance) {
               // If player is near edge, more likely to push
               const pushChance = isPlayerNearEdge ? 
                                 aiConfig.strategy.pushChanceNearEdge : 
                                 aiConfig.strategy.pushChanceNormal;
               const throwChance = isPlayerNearEdge ? 
                                  aiConfig.strategy.throwChanceNearEdge : 
                                  aiConfig.strategy.throwChanceNormal;
               const counterChance = aiConfig.strategy.counterChance;
               
               const randomAction = Math.random();
               
               // If player is attempting to throw, higher chance to counter
               if (this.player1.isThrowWindingUp && Math.random() < aiConfig.strategy.counterChanceVsThrow) {
                   if (this.player2.startCounter()) {
               ...
               // Otherwise, use normal action probabilities
                   ...
                   if (this.player2.isInCone(this.player1, gameConfig.throw.angle, gameConfig.throw.range)) {
                       if (this.player2.startThrow()) {
   ...
   resetRound() {
       ...
       if (this.gameMode === 'singlePlayer') {
           this.aiActionTime = gameConfig.ai.updateInterval.min;
       }
   }
}